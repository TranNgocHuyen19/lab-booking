...
const AdminJoinRequestDetailPage = () => {
  const { id } = useParams()
  ...
  const handleApprove = async (note: string) => {
    try {
      ...
      toast.success('Đã chấp nhận yêu cầu tham gia (Admin)')
  ...
  const handleReject = async (reason: string) => {
    try {
      ...
      toast.success('Đã từ chối yêu cầu tham gia (Admin)')
      setRejectOpen(false)
    } catch (error) {
      handleErrorApi({ error })
    }
  ...
  if (isRequestLoading) {
    return <LecturerLoading message='Đang tải thông tin đơn đăng ký (Admin)...' />
  }
    ...
    return (
      <div>
        <div className='text-center py-20 bg-white rounded-xl border border-dashed border-gray-300 shadow-sm'>
          ...
          <Button onClick={() => navigate(PATHS.ADMIN.APPROVALS.RESEARCH_GROUPS)}>Quay lại danh sách</Button>
        </div>
      </div>
    )
  }
            ...
            <div className='flex flex-col gap-1'>
              <div className='flex items-center gap-4'>
                ...
                <h1 className='text-2xl font-black tracking-tight text-primary uppercase'>
                  Chi tiết đơn tham gia nhóm (Admin)
                </h1>
                <Badge
              ...
              <p className='text-gray-500 font-medium ml-14'>
                Admin xem và xét duyệt đơn đăng ký tham gia nhóm nghiên cứu trên toàn hệ thống
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className='border-none shadow-sm rounded-xl overflow-hidden'>
          ...
          <CardContent className='p-8 space-y-12'>
              ...
              <section className='space-y-6 pt-8 border-t border-gray-100'>
                ...
                <div className='pt-4 flex items-center gap-2 text-sm font-semibold text-gray-500 italic justify-end'>
                  ...
                  <span className='underline underline-offset-2 capitalize text-gray-700 font-bold'>
                    {request.responseBy?.fullName || 'Hệ thống'}
                  </span>
                  <span>vào lúc</span>
            ...
            <div className='flex justify-end gap-3 pt-4'>
              ...
              <Button
                variant='cancel'
                onClick={() => navigate(PATHS.ADMIN.APPROVALS.RESEARCH_GROUPS)}
                className='px-10 h-11'
              >
                Đóng
              </Button>
